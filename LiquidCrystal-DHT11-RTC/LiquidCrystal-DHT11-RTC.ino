...
double hum = 0, temp = 0, heat_index ;
unsigned long printCounter = millis() ;
...
void loop() {
	...
	// Print Date or Humidity and Temperature
	lcd.setCursor(0, 0) ;

	if(millis() < printCounter + PRINTDELAY) {
		// Print Date
		sprintf(
			str2, "%s, %s %d%s",
		...
		center(str2, str, ' ', COL) ;	
		lcd.print(str) ;
	} else if(millis() < printCounter + PRINTDELAY * 2) {
		// Print temperature
	...
	} else if(millis() < printCounter + PRINTDELAY * 3) {
		// Print Humidity
		hum = dht.readHumidity();
		if (!isnan(hum)) {
			lcd.print(" Humidity ") ;
			dtostrf(hum, 3, 1, str2) ;
			lcd.print(str2) ;
			lcd.print("% ") ;
		}
	} else if(millis() < printCounter + PRINTDELAY * 4) {
		// Print heat index
		hum = dht.readHumidity();
		temp = dht.readTemperature();
